...
def create_template(header_list):
    header_template = np.ones((60, 256, 3), dtype=np.uint8)*255 # 6
    font_scale, font_thickness = 1.25, 3 # 6, 7
    # font_scale, font_thickness = 1, 2 # 1, 3

    # header_template = np.ones((30, 256, 3), dtype=np.uint8)*255
    # font_scale, font_thickness = 0.70, 2
    header_images = []
...
#         header_dict[item] = 1 if item not in header_dict else header_dict[item]+1
#     header_template = np.ones((50, 256, 3), dtype=np.uint8)*255
#     font_scale, font_thickness = 0.75, 2
#     header_images = []
#     for header in header_dict.keys():
#         img_header_ = [header_template.copy() for _ in range(header_dict[header])]
#         img_header_ = cv2.hconcat(img_header_)
#         # get size of text
#         line_width, line_height = cv2.getTextSize(header, cv2.FONT_HERSHEY_SIMPLEX, font_scale, font_thickness)[0]
#         left_space = (img_header_.shape[1] - line_width) // 2
#         top_space = (img_header_.shape[0] - line_height) // 2
...
def create_paper_fig(root, method_dict, case_id=None, sub_plot_size=None,max_row=20):
    ...
    for idx, input_image_path in enumerate(input_image_paths):
        ...
        for row_key in method_paths_dict.keys():
            ...
            cur_row_path= [method_paths_dict[row_key][input_image_name]] if input_image_name in method_paths_dict[row_key].keys() else ['/Users/minghaoliu/Desktop/HITL_navi/data/invalid.jpeg']

            # Use raw bitmoji assets
            flag_bitmoji = True
            for item in ['google','metahuman','NovelAI','/test/','other_AgileAvatar']:
                if item in cur_row_path[0]: flag_bitmoji = False
            if flag_bitmoji: 
                cur_row_path_debug = cur_row_path
                id = cur_row_path[0].split('.')[0].split('_')[-1]
                bitmojit_raw_root = '/Users/minghaoliu/Desktop/HITL_navi/data/asset/image_raw/'
                cur_row_path = [os.path.join(bitmojit_raw_root, id+'.png')]
                

            cur_row_paths += cur_row_path
...
def config(fig_id,mode='all'):
    ...
    if fig_id=='1':
        ...
        # paper cases
        if mode == 'paper':
            case_id2 = [84379,30688,17954,23617,34546,1940,86573,57130]
            case_id = [i for i in case_id if i not in case_id2]
            # case_id = [84379,30688,17954,23617,34546,1940,86573,57130] # Paper cases
        # case_id = [49712,26258,76308,39106,33176,23453] # Teaser
        ...
        # paper cases
        if mode == 'paper':
            case_id2 = [1965, 21869,23699,63323,64628,69329,8467,48508]
            case_id = [i for i in case_id if i not in case_id2]
            # case_id = [1965, 21869,23699,63323,64628,69329,8467,48508] # paper cases    
        ...
        method_dict = {
            'col1':['test'                       , ''                , 'Input'],
            'col2':['test_tag_pred'              , 'top1'            , 'Tag pred'],
            'col3':['test_direct_pred'           , 'resnet50_img_img', 'Direct pred'],
            'col4':['test_other_F2P_pred'        , 'pred'            , 'F2P'],
            'col5':['test_other_AgileAvatar_pred', 'pred'            , 'AgileAvatar'],
        }
    if fig_id=='6':
        case_id = [9096, 35957,1965,29172, 57282, 21658, 21869,51632,
        84379, 17712, 44945, 80522,57130, 23699,5085,40465,63055,42058,
        ...
        # paper cases
        if mode == 'paper':
            case_id2 = [15687,67028,85719,68293,50122,27397]
            case_id = [i for i in case_id if i not in case_id2]
            # case_id = [15687,67028,85719,68293,50122,27397] # paper cases
        case_id = [str(item) for item in case_id]

        # method_dict = {
        #     'col1':['test'        ,''                  , 'Input'   ],
        #     'col2':['test_tag_bd' ,'Top200/aggre_top1' , 'Low'           ],
        #     'col3':['test_tag_bd' ,'Top200/aggre_top2' , 'Low'    ],
        #     'col4':['test_tag_bd' ,'Top200/aggre_top3', 'Low'        ],
        #     'col5':['test_tag_bd' ,'Top200/aggre_top5', 'Low'   ],
        #     'col6':['test_tag_bd' ,'Top200/aggre_top99', 'High'          ],
        #     'col7':['test_tag_bd' ,'Top200/aggre_top123', 'High'          ],
        #     'col8':['test_tag_bd' ,'Top200/aggre_top140', 'High'          ],
        #     'col9':['test_tag_bd' ,'Top200/aggre_top170', 'High'          ],
        # }
        method_dict = {
            'col1':['test'        ,''                  , 'Input'   ],
            'col2':['test_tag_bd' ,'Top200/aggre_top1' , 'Low'           ],
    ...
    if fig_id=='7':
        ...
        # case_id1= [9096, 35957, 1965, 29172, 57282, 51632, 84379, 44945, 5085, 63323, 23920,
        ...
        # case_id1 = [str(item) for item in case_id1]

        # Using annotation results
        # method_dict = {
        #     'col1':['test'                                  ,''          , 'Input'     , None],
        ...
        # TODO: Fix the case id for this case
        case_id = [31179,69329,70430,39221,74223,20765,73518,81603,64628,12999,
        65730,60926,39502,26378,79326,10804,38226,20698,85595,79504,60533,52195,81086,8736,37280,2055,20765,11541,8346]
        # paper cases
        if mode == 'paper':
            case_id2 = [31179,69329,70430,39221,20765,81603,11541,8346]
            case_id = [i for i in case_id if i not in case_id2]
            # case_id = [31179,69329,70430,39221,20765,81603,11541,8346] # paper cases
        case_id = [str(item) for item in case_id]
        # case_id = None

        method_dict = {
            'col1':['test'                                  ,''          , 'Input'     , None],
            'col2':['test_tag_pred'                         ,'top1', 'Bitmoji'         , None],
            'col3':['other_system_pred/google_cartoon'      ,'top1'   , 'Cartoon set'  , None],
            'col4':['other_system_pred/metahuman_female'    ,'top1'   , 'Metahuman'       , 'Female'],
            'col5':['other_system_pred/metahuman_male'      ,'top1'   , 'Metahuman'       , 'Male'  ],
            'col6':['other_system_pred/NovelAI_male'        ,'top1'   , 'NovelAI'         , 'Male'  ],
            'col7':['other_system_pred/NovelAI_female'      ,'top1'   , 'NovelAI'         , 'Female'],
        }
...
if __name__ == "__main__":
    ...
    for fig_id in fig_ids:
        ...
        case_id = random.sample(case_id, 20)
