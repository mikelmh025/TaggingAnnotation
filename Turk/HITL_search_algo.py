...
def analysis_failed_dict(measure_dict):
    report = {}
    report_all = {}
    flip_counter = [0,0]
    for key in measure_dict:
        human_path = measure_dict[key]['input_human_']
        # human_path = input_human_.replace(url_root, data_root)
        options = measure_dict[key]['options']
        asset_paths = [item.replace(url_root, data_root) for item in options]

        human_id = human_path.split('/')[-1].split('.')[0]
        asset_ids = [item.split('/')[-1].split('.')[0].split('_')[1]+'.png' for item in asset_paths]

        # dist_dicts, dis_scores = [],[]
        # for asset_id in asset_ids:
        #     dis_dict_corr, dis_sum_corr = algo.eval_distance(human_data[human_id], asset_data[asset_id])
        #     # remove 0 scores itmes
        #     dis_dict_corr_ = {}
        #     for dis_dict_corr_item in dis_dict_corr:
        #         if dis_dict_corr[dis_dict_corr_item] !=0: dis_dict_corr_[dis_dict_corr_item] = dis_dict_corr[dis_dict_corr_item]
        #     dist_dicts.append(dis_dict_corr_)
        #     dis_scores.append(dis_sum_corr)

        asset_data_trimed = {key:value for key, value in asset_data.items() if key in asset_ids}
        search_scores, search_reports = algo.multi_round_search(human_data[human_id],asset_data_trimed)
        
        if asset_ids == list(search_scores.keys()):
            pass
        else:
            flip_counter[0] += 1
        flip_counter[1] += 1

        # TODO: report is not working yet, since multi_round_search trim out some asset_ids
        # report[key] = {'dist_dicts'   : {asset_id:search_reports[asset_id] for asset_id in asset_ids},
        #                 'test_scores' : search_scores,
        #                 'asset_tags'  : {item:asset_data_trimed[item] for item in asset_ids}
        #             }


        

    #     if dis_scores[0] < dis_scores[1]:
    #         report[key] = {'dis_dicts':dist_dicts, 'dis_scores':dis_scores,'human_tag':human_data[human_id], 'asset_tags':{item:asset_data[item] for item in asset_ids}}
    #     else:
    #         flip_counter[0] += 1
    #     report_all[key] = {'dis_dicts':dist_dicts, 'dis_scores':dis_scores,'human_tag':human_data[human_id], 'asset_tags':{item:asset_data[item] for item in asset_ids}}
    #     flip_counter[1] += 1
    # a=1
    return report, flip_counter, report_all

measure_dict = method1_fail_dict


rerpot_dir = 'improve_algo'
method1_fail_report, method1_fail_flip_counter,report_all1 = analysis_failed_dict(method1_fail_dict)
method2_fail_report, method2_fail_flip_counter,report_all2 = analysis_failed_dict(method2_fail_dict)
new_failed_report = {key:val for key,val in report_all2.items() if key not in method2_fail_report}
# both_failed_report = analysis_failed_dict(both_failed_dict)
# both_succeed_report = analysis_failed_dict(both_succeed_dict)
print('method1_fail keys', method1_fail_report.keys())

import json
# with open(os.path.join(rerpot_dir, 'method1_fail_report.json'), 'w') as f:
#     json.dump(method1_fail_report, f)
# with open(os.path.join(rerpot_dir, 'method2_fail_report.json'), 'w') as f:
#     json.dump(method2_fail_report, f)

# with open(os.path.join(rerpot_dir, 'new_failed_report.json'), 'w') as f:
#     json.dump(new_failed_report, f)

# with open(os.path.join(report_all1, 'report_all1.json'), 'w') as f:
#     json.dump(report_all1, f)

print('Good flip: method1_fail_flip_counter', method1_fail_flip_counter)
print('Bad flip:  method2_fail_flip_counter', method2_fail_flip_counter)

diff_flip = method1_fail_flip_counter[0] - method2_fail_flip_counter[0]
print('diff_flip: ', diff_flip)
